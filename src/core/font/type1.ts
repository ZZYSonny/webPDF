 ...
 * how the plan builds a face, but the program is also readable as itself.
 *
 * What the plan needs from it is the *names*, in glyph id order: a PDF's
 * `/Encoding` names a glyph (`/fi`), and the plan has to know which glyph id
 * that is before it can write `fi` as two letters (`font/encoding.ts`). The
 * charstrings and subrs come back too, and `tests/font-no-walk.mjs` uses them
 * to count a font's glyphs, but nothing in the viewer converts a charstring:
 * drawing one through FreeType is a few milliseconds a font and re-emitting the
 * curve is a charstring interpreter with hints, flex, `seac` and hint
 * replacement in it.
 *
...
function decrypt(cipher: Uint8Array, key: number): Uint8Array {
...
function splitEexec(bytes: Uint8Array): { text: string; start: number; end: number } | null {
  ...
  let at = -1;
  for (const m of text.matchAll(re)) at = (m.index ?? 0) + m[1].length;
  if (at < 0) return null;
  let start = at + 5;
...
export interface Type1Program {
  /** Charstring names in glyph id order; gid 0 is `.notdef`. */
  names: string[];
  /** Decrypted charstrings, in glyph id order. */
  charStrings: Uint8Array[];
  /** Decrypted subrs (`/Subrs`), by index. */
  subrs: Uint8Array[];
  /** The font's built-in encoding: character code -> glyph name. */
  encoding: Map<number, string>;
  /** `/Encoding` as the header declares it, when it is one of the standard ones. */
 ...
 */
export function parseType1(input: Uint8Array): Type1Program {
  ...
  const fontMatrix = header
    .match(/\/FontMatrix\s*\[([^\]]*)\]/)?.[1]
    ?.trim()
    .split(/\s+/)
    .map(Number) ?? null;
...
/** The charstring names in glyph id order, or null when the bytes are not Type 1. */
export function type1Names(bytes: Uint8Array): string[] | null {
  try {
    const names = parseType1(bytes).names;
 ...
 * the font's own encoding when it has one. One parse, because the caller that
 * wants one nearly always wants the other.
 */
export function type1Program(bytes: Uint8Array): { names: string[]; encoding: Map<number, string> } | null {
  try {
    const program = parseType1(bytes);
    if (program.names.length === 0) return null;
    return { names: program.names, encoding: program.encoding };
  } catch {
    return null;
  }
}
